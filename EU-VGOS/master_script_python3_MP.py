...
###     (BEWARE OF THE LEADING ZEROS "0" IN THE SCAN NAMES).
###
...
thesteps = {0: 'Source scanner',
            ...
            4: 'Calibrate bandpass (optional) and remove IONEX TEC',
            5: 'Perform broadband Global Fringe Fitting (optional)'}
...
### THEY CONTROL HOW THE DATA ARE CALIBRATED!   ###
###################################################
...
######################################
#####################
##### PARAMETERS INVOLVING ALL STEPS: 
# Number of processes allowed to run simultaneously:
NCPU = 4


# Name of the experiment (USED BY ALL STEPS):
EXPNAME = 'er2201'

# (sub)Directory with the ORIGINAL data:
DIFX_DIR = 'DiFX_2min'

# Destionation (sub)directory for the POLCONVERTED data (difx + metadata):
PCONV_DIR = 'DiFX_PCONV'

# LIST OF IFS TO CONVERT (STARTING FROM 1 !!!):
...
#######
# Destination (sub)directory of the BP+IONEX corrected data (difx + metadata):
...
######################################
#####################
##### PARAMETERS INVOLVING STEP 0: 

# NEW: List of IFs to use for the initial
#      coarse fringe-fitting (to estimate the SNR):
SCAN_IF = [4,12,20,28]
...
######################################
#####################
##### PARAMETERS INVOLVING STEPS 1 AND 2: 
...
# NEW: HOW DO WE INTERPOLATE THE X-Y PCAL DIFFERENCES?
XYPCALMODE = "bandpass" ## Can be "multitone" or "bandpass"


# NEW: APPLY UVTAPER (TO DOWNEIGHT LONG BASELINES IN X-POL 
#      GAIN ESTIMATES):
UVTAPER = 5.e6  ## IN METERS.
...
## NEW: LIST OF BAD IFs FOR GIVEN ANTENNAS (will zero the X-pol gains there):
...
# NEW: HAVING THE SAME ANTENNA CONFIGURATION IN ALL SCANS IS NO LONGER NEEDED.
POLCAL_SCAN = ['1']   # Little difference if we add more scans.
SUFFIX = '' ## Suffix to add to the difx polconverted subdirectories (not much needed).
#####################
######################################
...
######################################
#####################
##### PARAMETERS INVOLVING STEP 3: 

# SCAN(S) TO USE FOR THE PHASE ALIGNMENT AMONG IFs (NEW: NOW, THERE CAN BE MORE THAN ONE):
ADDITIVE_PHASE_SCANS = POLCAL_SCAN

# NEW: Calibrate the complex bandpass as well?
CALIB_BPASS = False


# NEW: TRANSLATION OF ANTENNA NAMES (FROM SWIN TO MRK4):
HOPSNAMES={'WF':'E','GS':'G','K2':'H','MG':'M','YJ':'Y','OE':'S','OW':'T','WS':'V','IS':'I'}


# NEW: SAMPLER DELAYS (IN MHz) IN THE "X" POL. CHANNEL, ORDERED BY BAND (FROM LOW TO HIGH):
...
# NEW: List of possible reference antennas,
# in order of preference:
GFF_REFANTS = ['OE','MG','YJ']
...
if 0 in mysteps:
    ...
    keyw = {'EXPNAME':EXPNAME, 
            'DIFX_DIR':DIFX_DIR, 
            'SCAN_IF':SCAN_IF,
            'NCPU':NCPU}

    keys = open('keywords_%s.dat'%SCRIPT_NAME,'wb'); 
    pk.dump(keyw, keys, protocol=0); keys.close()
...
if 1 in mysteps:
    ...
    for CURRIF in DOIF:
        ...
        keyw = {'EXPNAME':EXPNAME, 'DIFX_DIR':CALDIR, 'DOSCAN':POLCAL_SCAN, 'CHANSOL': CHAVG, 
            ...
            'PCAL_SUFFIX':'_IF%i'%CURRIF, 'IF_OFFSET':IF_OFFSET, 
	    'XYPCALMODE':XYPCALMODE, 'UVTAPER':UVTAPER}
...
## SECOND ROUND (APPLY)!!
    ...
    keyw = {'EXPNAME':EXPNAME, 'DIFX_DIR':CALDIR, 'XYGAINS':XYG, 'SUFFIX': SUFFIX, 
            'IF_OFFSET':IF_OFFSET, 'USE_PCAL':USE_PCAL, 'DOPLOT':True, 
            'SCAN_LIST':POLCAL_SCAN, 'REFANT':REFANT, 'XYPCALMODE':XYPCALMODE}
    keys = open('keywords_%s.dat'%SCRIPT_NAME,'wb'); 
    pk.dump(keyw, keys); keys.close()
    OFF = open('%s.py'%SCRIPT_NAME,'w')
    print(Start%SCRIPT_NAME,file=OFF)
...
if 3 in mysteps:
    ...
    for i,ADDITIVE_PHASE_SCAN in enumerate(ADDITIVE_PHASE_SCANS):
        ...
        keyw = {'SCAN':SCAN, 'HOPSNAMES': HOPSNAMES, 'IFNAMES': IFNAMES, 
                'FLAGBAS': EXCLUDE_BASELINE,'CALIB_BPASS':CALIB_BPASS,
                'PCALDELAYS': PCAL_DELAYS, 'REFANT':REFANT, 'FLAG_PCALS':FLAG_PCALS, 
                'IF_OFFSET':IF_OFFSET, 'SAMP_DELAYS':SAMP_DELAYS}
...
if 4 in mysteps:
    ...
    for SCAN in SCANS:
        ...
        keyw = {'EXPNAME':EXPNAME, 'SCAN':SCAN, 'ORIG_DIR':PCONV_DIR, 'DEST_DIR':BPCAL_DIR,
            'APPLY_PHASECAL':False, 'WRITE_DATA':True, 'FLAG_PCALS':FLAG_PCALS,
            'REFSCAN':ADDITIVE_PHASE_SCANS, 'REFANT':REFANT,
            'FLAGBAS': EXCLUDE_BASELINE, 'IF_OFFSET':IF_OFFSET,
...
if 5 in mysteps:
    ...
    for SCAN in SCANS:
        ...
        keyw = {'EXPNAME':EXPNAME, 'SCAN':SCAN, 'DIR':BPCAL_DIR,
            'APPLY_PHASECAL':False, 'FLAG_PCALS':FLAG_PCALS,
            'REFSCAN':ADDITIVE_PHASE_SCANS, 'REFANTS':GFF_REFANTS,
            'FLAGBAS': EXCLUDE_BASELINE, 'IF_OFFSET':IF_OFFSET,
            'SAMP_DELAYS':SAMP_DELAYS}
        keys = open('keywords_%s.dat'%SCRIPT_NAME,'wb'); 
        pk.dump(keyw, keys); keys.close()
