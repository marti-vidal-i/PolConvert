...
#ANTENNAS TO USE: GS, IS, K2, OE, OW, S6, T1, UM, WF, WN, WS, YJ
#ANTENNAS 2nd ITER: HV, KE, NN
#NEVER USE: YG
#NOT IN SCANS: HB

## AS NEW ANTENNAS ARE ADDED, SET HERE WHETHER THEIR PCALS
## ARE TO BE USED:
USE_PCAL = {'S6':False,'T1':False,'UM':True,'WN':True,'OE':True, 'OW':True, 'YJ':False,'IS':False,'WS':True,'WF':False,'GS':False,'K2':True,'MG':True,'HV':False,'KE':False,'NN':False,'YG':False}
...
#    os.system('cp -r %s %s/.'%('%s_%s.input'%(os.path.join(DIFX_DIR,EXPNAME),SI), CALDIR))
  ...
  for CURRIF in DOIF:
    ...
    BADANTS2 = [str(ai) for ai in BADANTS]
    for ant in ITER2:
      if ant not in BADANTS:
          BADANTS.append(str(ant))
    ...
    keyw = {'EXPNAME':EXPNAME, 'DIFX_DIR':CALDIR, 'DOSCAN':POLCAL_SCAN, 'CHANSOL': CHAVG, 
            ...
            'XPOL_DELAYS':XPOL_DELAYS,'USE_DELAY':USE_DELAY,'USE_RATE':USE_RATE,
    ...
    OFF = open('%s.py'%SCRIPT_NAME,'w')
    print(Start%SCRIPT_NAME,file=OFF)
    print('PolCal.POL_CALIBRATE(**kww)\n',file=OFF)
    if len(ITER2)>0:
        print('os.system(\"mv POLCAL_OUTPUT_SCAN-%s_IF-%i.dat POLCAL_TEMP_%s_%i.dat\")\n'%(POLCAL_SCAN[0],CURRIF,POLCAL_SCAN[0],CURRIF),file=OFF)
        print('IFF=open(\"POLCAL_TEMP_%s_%i.dat\",\"rb\"); gainIter1=pk.load(IFF); IFF.close()\n'%(POLCAL_SCAN[0],CURRIF),file=OFF)
        keyw2 = {'EXPNAME':EXPNAME, 'DIFX_DIR':CALDIR, 'DOSCAN':POLCAL_SCAN, 'CHANSOL': CHAVG, 
            'USE_PCAL':USE_PCAL, 'INTTIME':INTTIME, 'EXCLUDE_BASELINES':EXCLUDE_BASELINE,
            'DOAMP':ITER2DOAMP,'DOIF':[CURRIF],'PLOTANT':REFANT, 'APPLY_AMP':APPLY_AMP, 
            'EXCLUDE_ANTENNA':BADANTS2, 'SOLVER':SOLVER, 'APPLY_POLCAL':False, 
            'XPOL_DELAYS':XPOL_DELAYS,'USE_DELAY':USE_DELAY,'USE_RATE':USE_RATE,
            'FIT_ANTENNA':ITER2,
            'PCAL_SUFFIX':'_IF%i'%CURRIF, 'IF_OFFSET':IF_OFFSET, 'XYPCALMODE':XYPCALMODE}
        keys2 = open('keywords2_%s.dat'%SCRIPT_NAME,'wb'); pk.dump(keyw2, keys2,protocol=0); keys2.close()
        print('IFF = open(\'keywords2_%s.dat\',\'rb\'); kww2 = pk.load(IFF); IFF.close()\n'%SCRIPT_NAME,file=OFF)
        print('kww2[\"USE_GAINS\"]=gainIter1\n',file=OFF)
        print('PolCal.POL_CALIBRATE(**kww2)\n',file=OFF)
        print('os.system(\"mv POLCAL_OUTPUT_SCAN-%s_IF-%i.dat POLCAL_TEMP2_%s_%i.dat\")\n'%(POLCAL_SCAN[0],CURRIF,POLCAL_SCAN[0],CURRIF),file=OFF)
    OFF.close()
 ...
 # input("HOLD")
  ...
  for CURRIF in DOIF:
    if len(ITER2)>0:
      IFF1 = open("POLCAL_TEMP_%s_%i.dat"%(POLCAL_SCAN[0],CURRIF),"rb")
      GAINS1 = pk.load(IFF1); IFF1.close()
      if os.path.exists("POLCAL_TEMP2_%s_%i.dat"%(POLCAL_SCAN[0],CURRIF)):
        IFF2 = open("POLCAL_TEMP2_%s_%i.dat"%(POLCAL_SCAN[0],CURRIF),"rb")
        GAINS2 = pk.load(IFF2); IFF2.close()
        for ant in ITER2:
          GAINS1['XYadd'][ant] = GAINS2['XYadd'][ant]
          GAINS1['XYratio'][ant] = GAINS2['XYratio'][ant]
      OFF2 = open("POLCAL_OUTPUT_SCAN-%s_IF-%i.dat"%(POLCAL_SCAN[0],CURRIF),"wb")
      pk.dump(GAINS1,OFF2); OFF2.close()
...
#  sub2.set_ylim((0.,1.1*MaxG))
  ...
  if len(ITER2)>0:
     os.system('mv POLCAL_TEMP*_*.dat CROSS-POL_GAINS.DATA/.')
     os.system('mv keywords2_STEP1_*.dat STEP_KEYWORDS/.')
