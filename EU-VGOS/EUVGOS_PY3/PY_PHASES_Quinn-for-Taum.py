...
## We also estimate the bandpass phases.
  ...
  for bi in ALLBAS:
    ...
    for si in ALLSPW:
      ...
      PHASE_SPECTRUM[bistr].append(np.angle(np.average(VIS,axis=0)))
...
## NOW, ESTIMATE THE BANDPASS:
 # for spi in ALLSPW:
  ...
  for ai in ALLANTS:
    sub.cla()
    for si in ALLSPW:
      sub.plot(np.arange(128*si,128*(si+1)),GlobalBandpass[ai][si],'.')
...
## Write additive phases into external file:
  ...
  pk.dump([GlobalPhases,GlobalBandpass],PHSOFF)
  PHSOFF.close()
...
### Correct visibilities with GFF gains:
  ...
  for bi in ALLBAS:
    ...
    for si in ALLSPW:
      ...
      sub.plot(np.arange(128*si,128*(si+1)), 180/np.pi*np.angle(np.average(VIS,axis=0)),'.k')
...
## Apply PCALS, additive phases and Ionosphere corrections!
  ...
  for bi in ALLBAS:
    ...
    for spi in ALLSPW:
      mask2 = np.logical_and(mask,DATA['IF']==int(spi))
      DATA['VIS'][mask2,:] *= np.exp(1.j*( TWOPI*(PCALS[bi[0]][spi][0]-PCALS[bi[1]][spi][0])*(CHANFREQ[spi]-np.average(CHANFREQ[spi])) + PCALS[bi[0]][spi][1] - PCALS[bi[1]][spi][1] - (GlobalBandpass[bi[0]][spi]-GlobalBandpass[bi[1]][spi])*np.pi/180. - (GlobalPhases[bi[0]][spi]-GlobalPhases[bi[1]][spi])*np.pi/180. + TWOPI*0.*(TECOR[ANAMES[bi[0]]][1]-TECOR[ANAMES[bi[1]]][1])/CHANFREQ[spi]  ))
      avg_vis[bistr].append(np.average(DATA['VIS'][mask2*maskI,:],axis=0))
      sub.plot(np.arange(128*spi,128*(spi+1)),180/np.pi*np.angle(avg_vis[bistr][spi]))
      
      del mask2
      gc.collect()
    sub.set_ylim((-180,180))
    pl.savefig('phases_%s.png'%bistr)
    del mask
    gc.collect()
...
#Find the fringe peak for all baselines (and plot MBD fringe)

  fig = pl.figure(figsize=(12,6))
  sub= fig.add_subplot(111)

  del_tau = 1/(nu_max-nu_min)
  tau_min = -del_tau * n_nu/2
  tau_max = del_tau * n_nu/2
  tau_array = np.linspace(tau_min,tau_max,n_nu)
  ...
  for bi in ALLBAS:
    ...
    FRINGE_AMP = np.abs(all_mbfringe)
    peak=np.argmax(FRINGE_AMP)
    sub.cla()
    sub.plot(tau_array*1e6,FRINGE_AMP/FRINGE_AMP[peak],'-b')
    sub.set_xlabel(r'$\tau (\mu s)$')
    sub.set_xlim((-0.015,0.015))
    sub.set_ylabel(r'FRINGE AMPLITUDE')
    sub.set_ylim((0.,1.))
    sub.set_title('%s %s'%(bistr,os.path.basename(SCAN[:-5])))
    pl.savefig('%s_%s.png'%(os.path.basename(SCAN[:-5]),bistr))
  ...
  def globTEC(p):
    ...
    for bistr in avg_vis.keys():
      ...
      for IF in ALLSPW:
        ...
        GainPh = phi0 + taum*(CHANFREQ[IF]-6e9) + dTEC*CHORIZO*2*np.pi*(1-(CHANFREQ[IF])**2/numsq)/CHANFREQ[IF]
        resid += [np.cos(ObsPh)-np.cos(GainPh), np.sin(ObsPh)-np.sin(GainPh)]
...
#TODO Explore the effects of outliers (L1R)
  ...
  for i in ALLANTS:
    if i<REFID:
      pini[i-1] = (TECOR[ANAMES[i]][1]-TECOR[ANAMES[REFID]][1])/CHORIZO
    elif i>REFID:
      pini[i-2] = (TECOR[ANAMES[i]][1]-TECOR[ANAMES[REFID]][1])/CHORIZO
  ...
  for ai in ALLANTS:
    ...
    if ai!=REFID:
      ...
      print('dTEC (%i): %.3e'%(ai,GlobalTEC[ai]))
