    ...
    for bb in SNRBas:
        ...
        for IFp in DOIF:
            ...
            if np.sum(MASK) > 0:
                ...
                XXmatrix = np.fft.fftshift(
                    np.abs(np.fft.fft2(fringe2["VISIB"][MASK, :][XXp, :-1]))
                )
                XYmatrix = np.fft.fftshift(
                    np.abs(np.fft.fft2(fringe2["VISIB"][MASK, :][XYp, :-1]))
                )
                YXmatrix = np.fft.fftshift(
                    np.abs(np.fft.fft2(fringe2["VISIB"][MASK, :][YXp, :-1]))
                )
                YYmatrix = np.fft.fftshift(
                    np.abs(np.fft.fft2(fringe2["VISIB"][MASK, :][YYp, :-1]))
                )
                ...
                # Compute the std:
                SNR_XX /= np.sqrt(np.var(XXmatrix) + np.average(XXmatrix) ** 2.0)
                SNR_XY /= np.sqrt(np.var(XYmatrix) + np.average(XYmatrix) ** 2.0)
                SNR_YX /= np.sqrt(np.var(YXmatrix) + np.average(YXmatrix) ** 2.0)
                SNR_YY /= np.sqrt(np.var(YYmatrix) + np.average(YYmatrix) ** 2.0)
