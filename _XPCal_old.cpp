	...
	// First elements in line:       
     ...
     //  printf("Number of tones: %i\n",Aux); 
       ...
       while(std::getline(tempStr,auxStr,' ')){
	     ...
	     // (i.e., case 0, 1, 2, 3).
             ...
             case 0: nui = atof(auxStr.c_str());
               ...
               if (!RepNu && nui >0.0){PCalNus[Aux]=nui; j=Aux; Aux+=1; printf("NEW %.3f %i \n",nui,Aux);}; 
               i+= 1; break;
	  ...
	  // or total phase):	 
           switch(Ref){
               case 0: 
                     if(std::abs(PCalsX[l])>0.0){
                         PCalTemp = PCalsY[l]/PCalsX[l];
                     } else {
                         PCalTemp=1.0;
                     }; 
                   break;
               case 1: 
                     if(std::abs(PCalsY[l])>0.0){
                       PCalTemp = PCalsX[l]/PCalsX[0];
                     } else {
                       PCalTemp=1.0;
                     }; 
                   break;
...
if (!PCalsY || !PCalsX || !PCalsD || !PCalNus || 
    !PCalsAY || !PCalsAX || !RateFitXp || !RateFitXt ||
    !RateFitXpt || !RateFitXtt || !PrevPhX || !NWrap ||
    !PDi || !Ti || !NTimes || !TBUFF ||
    !NGoods || !goodY || !goodX) return ret;
